...
class LoginModel {
	...
	public function rememberToken($datos) {
		...
		$result = $this->mysqlDB->where ('email')->select ()->from ( $this->table )->fetchall ();
				
		// generate cookie string that consists of user id, random string and combined hash of both
		$cookie_string_first_part = $result[0]['id_usuario'] . ':' . $random_token_string;
		$cookie_string_hash = hash ( 'sha256', $cookie_string_first_part );
		echo "cookie string hash es ".$cookie_string_hash;
